    ...
    <main dir="rtl" style={themeVars}
      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-6 px-4 pb-24">
        <section className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3">
          {filtered.map(r => <RuleCard key={r.id} rule={r} onOpen={()=>openRule(r)} />)}
          {filtered.length===0 && <div className="col-span-full rounded-2xl border border-[color:var(--glass-border)] bg-[color:var(--glass)] p-6 text-center backdrop-blur-xl">هیچ موردی یافت نشد.</div>}
        </section>
      </div>
  ...
  return (
    <article className="group relative overflow-hidden rounded-3xl border border-[color:var(--glass-border)] bg-[color:var(--glass)] p-5 shadow-xl backdrop-blur-2xl transition sm:hover:scale-[1.01] hover:shadow-2xl transform-gpu [will-change:transform]">
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <button onClick={onOpen} className="cursor-pointer rounded-full border border-[color:var(--glass-border)] bg-[color:var(--glass-strong)] px-3 py-1 text-xs backdrop-blur-md transition hover:brightness-110 transform-gpu [will-change:transform]">توضیح بیشتر</button>
      </div>
    </article>
  );
}
      ...
      <div className="mx-auto w-full max-w-3xl">
        <article className="relative overflow-hidden rounded-3xl border border-[color:var(--glass-border)] bg-[color:var(--glass)] p-6 shadow-2xl backdrop-blur-xl">
          ...
          <div className="relative flex items-center justify-between gap-3">
            ...
            <button onClick={onBack} className="relative z-10 rounded-full border border-[color:var(--glass-border)] bg-[color:var(--glass-strong)] px-3 py-1 text-xs backdrop-blur-md transition hover:brightness-110 transform-gpu [will-change:transform]">بازگشت</button>
          </div>
...
function BgLayer({ theme, mouse }: { theme: "light"|"dark"; mouse:{x:number;y:number} }) {
  ...
  const t = (k:number)=>({ transform:`translate(${(mouse.x-dims.w/2)/k}px, ${(mouse.y-dims.h/2)/k}px)` });
  return (
